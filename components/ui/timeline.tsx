...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-black mb-20  md:px-10 flex flex-col items-center justify-center bg-grid-light"
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row  items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-pink-500  ">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full ">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-pink-500 ">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
